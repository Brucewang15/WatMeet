...
# Dragon Boat
def scrape_dragon_boat_club_page(url, output_filename="artistic_swimming.json"):
    # Initialize the Selenium WebDriver
    driver = webdriver.Chrome()
    ...
    if os.path.exists(output_filename):
        with open(output_filename, "r+", encoding="utf-8") as json_file:
            ...
            if "Aquatic Sports" not in data:
                data["Aquatic Sports"] = []
            data["Aquatic Sports"].append(dragon_boat_data)
            json_file.seek(0)
            json.dump(data, json_file, ensure_ascii=False, indent=4)
    else:
        with open(output_filename, "w", encoding="utf-8") as json_file:
            json.dump({"Aquatic Sports": [dragon_boat_data]}, json_file, ensure_ascii=False, indent=4)

    print(f"Data for Dragon Boat Club added to {output_filename}")

# Example usage
scrape_dragon_boat_club_page("https://athletics.uwaterloo.ca/sports/2023/9/21/rec-landing-page-dragon-boat-club.aspx")
